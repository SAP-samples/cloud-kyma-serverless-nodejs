...
class ProduceStatistics extends EventEmitter {
    ...
    constructor(acks, maxCount, logCount) {
        ...
        this._countAckTodo = acks ? maxCount : 0;
...
class ConsumeStatistics extends EventEmitter {
    ...
    onReceive() {
        ...
        if (this._countDone < this._countTodo) {
            ...
            this.emit('info', this._countDone);
    ...
    print(info) {
        if (this._timeFinish === 0) this._timeFinish = Date.now();
 ...
 * @private
 */
class ForwardStatistics extends Transform {
    ...
    _transform(message, encoding, callback) {
        ...
        if (++this._countSent <= this._countTodo) {
            if (this._countSent % this._countLog === 0) this.emit('info', this._countSent);
            const done = message.done;
            message.done = () => this._complete(done);
            const failed = message.failed;
            message.failed = () => this._complete(failed);
...
module.exports = {
    ...
    ForwardStatistics,
    JSONValidator,
    jsonPayload : Buffer.from('{"a1":10,"a2":"42","a3":"test","a4":{"b":42}}'),
    dataPayload : Buffer.from('aaaaaaaaaa.aaaaaaaaaa.aaaaaaaaaa.aaaaaaaaaa'),
};
